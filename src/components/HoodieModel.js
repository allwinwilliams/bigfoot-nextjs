...
const HoodieModel = ({ color, texture, triggerAnimation = false, animationDuration = 4, triggerLoadingAnimation = false }) => {
  const gltf = useLoader(GLTFLoader, '/models/hoodie-model.glb');
  const normalMapUrl = '/models/textures/hoodie-normal.png';
  ...
  useEffect(() => {
    if (gltf && texture) {
      ...
      gltf.scene.traverse((child) => {
        if (child.isMesh && child.name === 'Object_3') {
  ...
  return <primitive ref={modelRef} object={gltf.scene} position={[0, 0, 0]} scale={[0.2, 0.2, 0.2]} />;
};
